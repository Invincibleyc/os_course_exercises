...
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   ...
   答：是指通过合理地进行分配，使得各个存储层次都处于繁忙状态，而整个系统能够流畅运行。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

   答：遇到过内存不够的情况。解决方案是降低程序的空间复杂度。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

   答：对程序按照相对独立的功能划分成模块，让相互之间没有调用关系的模块共享一块内存区域。

   程序员需要进行模块的划分。

2. 什么是交换技术？覆盖与交换有什么不同？

   答：把暂时不用的进程的整个地址空间保存至外存，将另一个进程换入。

   覆盖是解决了程序所需存储空间大于物理空间的问题，而交换技术解决了多个进程运行时空间不够的问题。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   答：时间局部性：在一定时间内，曾经访问过的内存很可能再一次访问。空间局部性：曾经访问过的空间，其附近的空间很可能被访问。分支局部性：前一次分支预测的结果很可能和当前的结果相同。

2. 如何提高程序执行时的局部性特征？

   答：在程序编写过程中，有意识地规定空间的访问顺序。（例如二维数组的遍历）
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   答：把不常用的内存块放到外存中。不需要考虑程序内部的复杂调用关系。

   好处：可以不连续

2. 虚拟存储需要什么样的支持技术？

   答：操作系统需要支持内存的换入换出，硬件需要支持地址转换
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    ...
    答：虚拟页式存储：将置换的基本单位设置为页。

    缺页中断处理：将对应的外存当中的页面调入内存，并修改相应的页表项，然后重新执行指令。
...
1. 缺页异常的处理流程？
   ...
   答：指令执行中的页表项引用->由于页面不在内存，导致缺页异常->在外存中查找需要访问的页面备份->页面换入->页表项修改->重新执行导致缺页异常的指令
